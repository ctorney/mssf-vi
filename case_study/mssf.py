...
class ConvergenceCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        print("current beta mean: ", current_variables[0])
        print("current beta std: ", current_variables[1])
...
class stepSelectionVI(tf.keras.Model):
    def __init__(self, n_covars, cov_tensor, x_ref_min, x_ref_max, move_std=1.0,   prior_mean=None, prior_std=None, n_gh_points=4, n_gh_points_vi=4):
    ...
    def train_step(self, data):
        ...
        for g in gradients:
            tf.debugging.assert_all_finite(g, "Gradients are not finite")
        ...
        return {"loss": self.loss_tracker.result()}#,"beta_means": self.beta_mean, "beta_std": self.beta_std[0,:], "move_std": self.move_std}
    ...
    def log_likelhood(self, beta_params, start_points, end_points, step_times):
        ...
        sigmas = self.move_std*tf.math.sqrt(0.5*step_times)
