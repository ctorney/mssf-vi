...
@tf.function
def onestep(xyt, stdt):
    C = tf.random.normal(shape=(nrepeat, 2), mean=xyt, stddev=stdt[:, None])
    grid = tf.random.normal(shape=(nrepeat, npts, 2),
                            mean=C[:, None, :], stddev=stdt[:, None, None])

    zgrid = grid % L
    cov_xy_end = tfp.math.batch_interp_regular_nd_grid(
        zgrid, x_min, x_max, cov_tensor, axis=-3)
    allrsf = tf.reduce_sum(tf.multiply(cov_xy_end, beta_tensor), axis=-1)
...
for nbObs in [10001, 100001, 1000001]:
    for beta in [[[0.5, -0.8]], [[-1.5, -1.8]], [[-1.5, 1.8]], [[1.2, 1.8]]]:
        ...
        for t in tqdm(range(1, nbObs)):
            ...
            next_points = onestep(xy[t-1], stds[:, t-1])
        ...
        for repeat in range(nrepeat):
            pos_filename = "pos_N_" + str(nbObs-1) + "_b1_" + str(
                beta[0][0]) + "_b2_" + str(beta[0][1]) + "_repeat_" + str(repeat) + ".csv"
            grid_filename = "cov_N_" + str(nbObs-1) + "_b1_" + str(
