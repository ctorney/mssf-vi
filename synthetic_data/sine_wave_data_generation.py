...
cov_field = np.sin(2*np.pi/len_scale_1*positions[:, 0]) 


cov_cube = cov_field.reshape(1,1,51, 51)
    ...
    for beta in [[[0.1]]]:
        ...
        for repeat in range(nrepeat):
            pos_filename = data_dir + "/pos_N_" + str(nbObs-1) + "_b1_" + str(
                beta[0][0]) + "_repeat_" + str(repeat) + ".csv"
            grid_filename = data_dir + "/cov_N_" + str(nbObs-1) + "_b1_" + str(
                beta[0][0]) + "_repeat_" + str(0) + ".npy"
            ...
            np.save(grid_filename, cov_cube[0])
