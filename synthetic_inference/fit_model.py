...
ssf = stepSelectionVI(2, cov_tensor, move_std=1.00,
                      L=50.0, n_gh_points=3, n_vi_samples=64)
...
train_dataset = train_dataset.shuffle(
    buffer_size=64).batch(batch_size, drop_remainder=True)
...
lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
    0.1, decay_steps=100, decay_rate=0.9, staircase=True)
...
def plot_fit():  # data_batch
    ...
    plt.plot(x, y, c='C0')
    plt.hist(chain1[:, 0, 0], bins=20, density=True, color='C1')
...
ssf.fit(train_dataset, epochs=5)
