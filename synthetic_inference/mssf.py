...
class stepSelectionVIuselesspieceofcrap(tf.Module):
    def __init__(self, n_covars, cov_tensor, move_std=1.0, L=1.0, prior_mean=None, prior_std=None,n_gh_points=4,n_vi_samples=4):
        ...
        self.beta_std = tfp.util.TransformedVariable([np.eye(n_covars,dtype=np.float32)],tfp.bijectors.FillScaleTriL(diag_shift=np.float32(1e-5)),dtype=tf.float32)
